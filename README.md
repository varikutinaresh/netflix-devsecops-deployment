...
pipeline {
    ...
    stages {
        ...
        stage('Checkout from Git') {
            steps {
                git branch: 'main', url: 'https://github.com/varikutinaresh/netflix-devsecops-deployment.git'
            }
        }
        ...
        stage("Docker Build Image"){
            steps{
                ...
                sh "docker build --build-arg API_KEY=2af0904de8242d48e8527eeedc3e19d9 -t netflixin ."
            }
        }
        stage("TRIVY"){
            steps{
                sh "trivy image netflixin > trivyimage.txt"
                script{
                    input(message: "Are you sure to proceed?", ok: "Proceed")
                }
            }
        ...
        stage("Docker Push"){
            steps{
                script {
                    ...
                    sh "docker tag netflixin vnareshreddy/netflixin:latest "
                    sh "docker push vnareshreddy/netflixin:latest"
                    }
                }
            }
        }
